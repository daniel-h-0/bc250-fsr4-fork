...
def icd(library):
    return {'file_format_version': '1.0.0', 'ICD': {
        'library_path': str(library), 'api_version': '1.4.0'}}
...
def check_hardware():
    ...
    for device in Path('/sys/bus/pci/devices').glob('*'):
        try:
            if (device/'vendor').read_text().strip() == '0x1002' and (device/'device').read_text().strip() == '0x13fe':
                return
...
def probe(library, directory):
    ...
    if linked.returncode or any(message in linked.stdout + linked.stderr for message in ('not found','undefined symbol')):
        raise RuntimeError('Binary ABI/dependency check failed. Build on this distribution.\n' + linked.stdout + linked.stderr)
    ...
    env['LD_BIND_NOW'] = '1'
    result = subprocess.run(['vulkaninfo', '--summary'], env=env, capture_output=True, text=True, timeout=60)
    (directory/'probe.log').write_text(result.stdout + result.stderr)
    if result.returncode or 'radv' not in result.stdout.lower() or 'gfx1013' not in result.stdout.lower():
        raise RuntimeError('BC250 RADV initialization failed. Current release is unchanged.\n' + result.stdout + result.stderr)
    return {'loader': 'vulkaninfo --summary (LD_BIND_NOW=1)', 'success': True, 'dependencies': linked.stdout, 'vulkaninfo': result.stdout}
    ...
    with tarfile.open(archive) as bundle:
        ...
        for member in members:
            if not (member.isfile() or member.isdir()):
                raise RuntimeError('Release archives may contain only regular files and directories.')
...
def pending(prefix):
    return [(p, json.loads(p.read_text())) for p in sorted((prefix/'transactions').glob('*.json')) if json.loads(p.read_text()).get('state') in ('prepared','rolling-back')]


def install(args, prefix):
    if pending(prefix):
        raise RuntimeError('An interrupted transaction needs recovery: run driver.py recover first.')
    ...
    if stable_icd.exists() and json.loads(stable_icd.read_text()) != icd(prefix/'current/lib/libvulkan_radeon.so'):
    ...
    for raw in args.upgrade_v3_icd:
        ...
        if path.name not in ('radv-bc250-fsr4-v3.json', 'radv-bc250-fsr4.json') or library.name != 'libvulkan_radeon.so' or not library.is_file():
    ...
    with tempfile.TemporaryDirectory(prefix='.stage-', dir=prefix) as temporary:
        ...
        release_id = manifest['version'] + '-' + manifest['driver_sha256'][:12] + '-' + digest(root/'release.json')[:8]
        if any(c not in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.-' for c in release_id):
        ...
        for item in migrations:
            after = (json.dumps(icd(prefix/'current/lib/libvulkan_radeon.so'), indent=2) + '\n').encode()
            item['after_sha256'] = hashlib.sha256(after).hexdigest()
            item['after_hex'] = after.hex()
        journal = {'schema': 1, 'previous': previous, 'target': target, 'icd': str(driver_icd), 'migrations': migrations, 'probe': result, 'archive_sha256': checksum, 'state': 'prepared'}
        transactions = prefix/'transactions'
        transactions.mkdir(exist_ok=True)
        record = transactions/(time.strftime('%Y%m%dT%H%M%S') + '-' + str(time.time_ns()) + '.json')
    ...
    print('Steam launch option: VK_DRIVER_FILES=' + __import__('shlex').quote(str(prefix/'current.json')) + ' %command%')
...
def rollback(prefix):
    ...
    for item in journal['migrations']:
        if digest(item['path']) != item['after_sha256']:
            raise RuntimeError('Legacy ICD was modified after installation; preserving it: ' + item['path'])
    ...
    write_json(record, journal)
    print('Previous selection and legacy ICD files restored. Release payloads retained. Restart games.')


def recover(prefix):
    records = pending(prefix)
    if len(records) != 1:
        raise RuntimeError('Recovery expects exactly one interrupted transaction, found ' + str(len(records)))
...
def status(prefix):
    ...
    if json.loads((prefix/'current.json').read_text()) != icd(prefix/'current/lib/libvulkan_radeon.so'):
        raise RuntimeError('Stable ICD differs from the managed selection.')
    return {'active': True, 'prefix': str(prefix), 'version': manifest['version'], 'driver_sha256': manifest['driver_sha256'], 'library': str(prefix/target/'lib/libvulkan_radeon.so'), 'scope': 'Private 64-bit selection; use the wrapper or printed ICD. Does not change system RADV.'}


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--prefix', type=Path, default=Path(os.environ.get('BC250_FSR4_PREFIX',str(Path.home()/'.local/share/bc250-fsr4'))))
    ...
    if requested_prefix.is_symlink() or prefix in [Path('/'), Path.home(), Path('/usr'), Path('/etc'), Path('/tmp')]:
        raise RuntimeError('Choose a dedicated installation directory, not a system/home root or symlink.')
