...
def main():
    ...
    parser.add_argument('action', choices=['status', 'rollback', 'check-update'], nargs='?', default='status')
    parser.add_argument('--json', action='store_true', help='Compatibility option; status is always JSON')
    args = parser.parse_args()
    metadata = json.loads(Path('/usr/share/bc250-fsr4-v4/system.json').read_text())
    active = Path(metadata['library']).is_file() and sha(metadata['library']) == metadata['driver_sha256']
    if args.action == 'rollback':
        ...
        if not active:
            raise RuntimeError('Driver changed since v4 installation; refusing to downgrade a newer or locally modified package.')
        ...
        return 0
    print(json.dumps({'active': active, 'drivers': {metadata['library']: active}, **metadata}, indent=2))
    if not active:
        print('The distribution or another install replaced v4. Rebuild/revalidate for the current Mesa version; do not copy an old driver over an updated package.')
