...
def build(args):
    ...
    if ".INSTALL" in call(["bsdtar", "-tf", base]).splitlines():
        raise RuntimeError(
            "Base package has install scriptlets; this overlay route cannot safely preserve their semantics."
        )
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        if mesa != release["mesa"]:
            raise RuntimeError(
                "Mesa version mismatch: rebuild/rebase before replacing another Mesa version."
            )
    ...
    old_library = subprocess.check_output(
        ["bsdtar", "-xOf", str(base), "usr/lib/libvulkan_radeon.so"]
    )
    metadata = {
        "schema": 1,
...
"""
    ...
    # User makepkg settings may otherwise redirect artifacts elsewhere or use
    # another archive extension, making a successful build look incomplete.
    environment.update(PKGDEST=str(output), BUILDDIR=str(output), PKGEXT=".pkg.tar.zst")
...
def install(args):
    ...
    if (
        call(["pacman", "-Q", "vulkan-radeon"]).strip()
        != "vulkan-radeon " + metadata["base_version"]
    ):
        raise RuntimeError(
            "Installed RADV version differs from the retained base package. Regenerate from the matching archive; do not force it."
        )
    if driver.digest(metadata["library"]) != metadata["base_driver_sha256"]:
        raise RuntimeError(
            "Installed driver does not match the rollback archive. For an active v4 update, first use bc250-fsr4 rollback, then install the new reviewed pair. Preserve/reconcile any other local modification first."
        )
    packages = []
