...
def change_arrow(x1, x2, y, resolution, source_backend, source_mode, target_backend, target_mode, lower=False):
    ...
    for offset, value, size, color in [(0, label, 27, CHANGE), (28, ms_label, 23, MUTED)]:
        ...
        annotation.set_bbox({'facecolor': BG, 'edgecolor': 'none', 'pad': 2})
...
for x in [margin, left, left + gw, right]:
...
modes = [('Performance', 'Performance'), ('Balanced', 'Balanced'),
         ('Quality', 'Quality'), ('Native', 'Native AA (1:1)')]
for i, (label, mode) in enumerate(modes):
    y = body + i * row_h
    if i:line(margin, y, right, y)
    text(78, y + 32, label, 27)
    for x, _, resolution in groups:
        if i == 3:
            fps(x + .5 * cw, y + 32, resolution, 'Off', 'Native, no AA')
            change_arrow(x + .5 * cw, x + 1.5 * cw, y, resolution,
                         'Off', 'Native, no AA', 'FSR3', mode)
        ...
        change_arrow(x + 1.5 * cw, x + 2.5 * cw, y, resolution,
                     'FSR3', mode, 'FSR4 RC11 INT8', mode)
        if i == 3:
            change_arrow(x + .5 * cw, x + 2.5 * cw, y, resolution,
                         'Off', 'Native, no AA', 'FSR4 RC11 INT8', mode, lower=True)
...
halo[:, :, 3] = np.clip(near * .65 + far * .4, 0, 255).astype(np.uint8)
...
(ROOT / 'render-manifest.json').write_text(json.dumps({
    ...
    'png_pixels': [W * 2, H * 2], 'theme': 'dark slate',
